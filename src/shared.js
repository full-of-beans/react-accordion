...
function TabItem({ position, isOpen, ...props }) {
  ...
  return position === 'above' ? (
    <AboveTabItem {...modifiedProps} />
  ) : (
    <BelowTabItem {...modifiedProps} />
  );
}

const preventClose = (state, changes) =>
  changes.type === 'closing' && state.openedIndexes.length < 2
    ? { ...changes, openedIndexes: state.openedIndexes }
    : changes;

const single = (state, changes) =>
  changes.type === 'opening'
    ? { ...changes, openedIndexes: changes.openedIndexes.slice(-1) }
    : changes;
